...
## Case study: is height hereditary?{.smaller}

- The Pearson correlation coefficient is an informative summary of how two variables move together.

- we can define the sample correlation for any bivariate set of data
...
$$ 
- with $\bar{x}, \bar{y}$ the sample means of $x_1,\dots, x_n$ and $y_1, \dots, y_n$, respectively, and $s_x, s_y$ the sample standard deviations.
...
- It is not a coincidence that $r$ is the first letter in _regression_.

- We will learn about the connection between correlation and regression.
...
- To understand why this equation does in fact summarize how two variables move together, note that the $i$-th entry of $x$ is $\left( \frac{x_i-\bar{x}}{s_x} \right)$ SDs away from the average.

- Similarly, the $y_i$ that is paired with $x_i$, is $\left( \frac{y_1-\bar{y}}{s_y} \right)$ SDs away from the average $y$.

- If $x$ and $y$ are unrelated, the product 
$$ 
\left( \frac{x_i-\bar{x}}{s_x} \right) \cdot \left( \frac{y_i-\bar{y}}{s_y} \right) 
$$ 
will be positive ( $+ \times +$ and $- \times -$ ) as often as negative ($+ \times -$ and $- \times +$) and will average out to about 0.

- this correlation measure linear relationships and is less informative if $x$ and $y$ have a non-linear relationship
...
- The Pearson correlation is the average of the product of those deviations and therefore unrelated variables will have a correlation close to zero

- If instead the quantities vary together, say higher values of x go with higher values of y, then we are averaging mostly positive products so we get a positive correlation

- If large values of x go with smaller values of y, then we are averaging mostly negative values, and we get a negative correlation
...
- We can show these facts mathematically.
...
- The function `cor(x, y)` computes the sample correlation, which divides the sum of products by `length(x)-1` rather than `length(x)`.

- The rationale for this is akin to the reason we divide by `length(x)-1` when computing the sample standard deviation `sd(x)`.


- Namely, this adjustment helps account for the degrees of freedom in the sample, which is necessary for unbiased estimates.
...
```{r sample-correlation-distribution-run, fig.align = "center"} 
#| echo: true
...
- $N=25$ does not seem to be large enough to get a good approximation to the Normal distribution:

```{r small-sample-correlation-not-normal-run, fig.align = "center"} 
#| echo: true
#| output-location: slide
...
- In the previous chapter, we learned that if you take a random pair $(X,Y)$, the expected value and best predictor of $Y$ is $\mbox{E}(Y) = \bar{y}$, the population average $1/n\sum_{i=1}^n y_i$.

- However, we are no longer interested in the general population.

- We want to see if we can get better estimates if we know something additional, say from another variable that is correlated with $Y$.
...
- Note that a 72 inch father is taller than average, specifically (72.0 - `r round( mean(galton_heights$father),1)`)/`r round(sd(galton_heights$father),1)` = `r round((72 -mean(galton_heights$father))/sd(galton_heights$father), 1)` standard deviations taller than the average father.
...
```{r boxplot-1-run, fig.align = "center", fig.height = 3, fig.width = 3, out.width="40%", echo=FALSE} 
...
```{r conditional-averages-follow-line, fig.align = "center", echo=FALSE, fig.height = 3, fig.width = 3, out.width="40%"} 
galton_heights |>  
...
- then we predict that for every standard deviation, $\sigma_X$, that $x$ increases above the average $\mu_X$, 
- our prediction $\widehat{Y}$ increase $\rho$ standard deviations $\sigma_Y$ above the average $\mu_Y$ with $\rho$ the correlation between $X$ and $Y$.
...
$$ 
\left( \frac{\widehat{Y}-\mu_Y}{\sigma_Y} \right) = \rho \left( \frac{x-\mu_X}{\sigma_X} \right) 
$$
...
$$ 
\widehat{Y} = \mu_Y + \rho \left( \frac{x-\mu_X}{\sigma_X} \right) \sigma_Y 
$$

- If there is perfect correlation, the regression line predicts an increase that is the same number of SDs.

- If there is 0 correlation, then $X$ does not help predict $Y$ at all we simply predict the average $\bar{Y}$.

- For values of $|\rho|$ between 0 and 1, the prediction is somewhere in between.

- If the correlation between $X$ and $Y$ is negative, we predict a reduction instead of an increase.
...
$$
\widehat{Y} = b + mx \mbox{ with slope } m = \rho \frac{\sigma_y}{\sigma_x} \mbox{ and intercept } b=\mu_y - m \mu_x 
$$
...
```{r regression-line, fig.align = "center", fig.height = 3, fig.width = 3, out.width="40%", eval=FALSE} 
...
```{r regression-line-run, fig.height = 3, fig.width = 3, out.width="40%", echo=FALSE} 
mu_x <- mean(galton_heights$father) 
mu_y <- mean(galton_heights$son) 
s_x <- sd(galton_heights$father) 
s_y <- sd(galton_heights$son) 
...
```{r two-regression-lines-run, fig.align = "center", fig.height = 3, fig.width = 3, out.width="40%", echo=FALSE} 
galton_heights |>  
  ggplot(aes(father, son)) +  
  geom_point(alpha = 0.5) +  
...
- We are now ready to understand the connection between regression and _linear models_.
...
- We note that _linear_ here does not refer to lines exclusively, but rather to the fact that the conditional expectation is a linear combination of known quantities.

## Linear models

- In mathematics, when we multiply each variable by a constant and then add them together, we say we formed a _linear combination_ of the variables.
...
- Once we find the values that minimize the RSS, we will call the values the least squares estimates (LSE) and denote them by placing a _hat_ over the parameters.
...
```{r rss-versus-estimate-run, echo=FALSE} 
...
## LSE are random variables{.smaller}

- Under this assumption, mathematical theory tells us that the LSE divided by their standard error,
$$
\hat{\beta}_0 / \widehat{\mbox{SE}}(\hat{\beta}_0 ) \quad \mbox{and} \quad
\hat{\beta}_1 / \widehat{\mbox{SE}}(\hat{\beta}_1 )
$$
follow a t-distribution with $N-p$ degrees of freedom, with $p$ the number of parameters in our model.
...
- When we plot $\widehat{Y}$ versus $x$, we see the regression line.

- Keep in mind that the prediction $\widehat{Y}$ is also a random variable and mathematical theory tells us what the standard errors are.
...
- In fact, the **ggplot2** layer `geom_smooth(method = "lm")` that we previously used plots $\widehat{Y}$ and surrounds it by confidence intervals:

```{r father-son-regression, eval=FALSE} 
...
```{r father-son-regression-run, echo=FALSE} 
galton_heights |> ggplot(aes(son, father)) + 
  geom_point() + 
  geom_smooth(method = "lm") 
``` 
...
$$
r = Y - \widehat{Y} = Y - \left(\widehat{\beta}_0 - \widehat{\beta}_1 x_i\right), 
$$
...
- In fact residuals are often denoted as $\widehat{\varepsilon}$.

- This motivates several _diagnostic_ plots.
...
- Because we observe $r$, but don't observe $\varepsilon$, we based the plots on the residuals.
  ...
  1. Because the errors are assumed not to depend on the expected value of $Y$, a plot of $r$ versus the fitted values $\widehat{Y}$ should show no relationship.
...
- [Sophomore Slump](https://www.baseball-reference.com/bullpen/Sophomore_slump)

- You can find the data in this R package: [Lahman](https://cran.r-project.org/web/packages/Lahman/index.html)
...
- In fact, the proportion of players that have a lower batting average during their sophomore year is `r mean(ROY$sophomore - ROY$rookie <= 0)`.
...
## The regression fallacy{.smaller}

- The correlation is `r round(cor(two_years$"2013",two_years$"2014"), digits=3)` and the data look very much like a bivariate normal distribution, which means we predict a 2014 batting average $Y$ for any given player that had a 2013 batting average $X$ with:

...
```{r, ThemePark, message=FALSE, warning=FALSE, echo=FALSE, eval=FALSE}
...
* we can also use the `lattice` package to produce high quality graphics - R has lots of ways to do things

```{r lattice ex}
pl <- xyplot(total ~ population/10^6 | region, data = murders,
      type = c("p", "g"), xlab = "Population/10^6", ylab = "Total",
...
* here we produce a similar plot using `ggplot2`
```{r ggplotpanel}

...
* The things between the `---` are the [YAML](https://www.cloudbees.com/blog/yaml-tutorial-everything-you-need-get-started) header.
...
* The main reason we use Quarto is because we can include and execute the code when compiling the document. 

* In R we refer to the code as R chunks.

* This applies to plots as well; the plot will be placed in the position of the code. 
...
`format: html`, `format: pdf`,`format: docx`, or `format: gfm` 
